...
\usepackage{amsmath,listings, graphicx, caption, subcaption, float}
...
Furthermore, we need to take into account that $u \rightarrow v$ could potentially have non-trivial homology $\vec{[h]}$ and $\alpha$ term, in which case we need to define what it means to split $uv$ to two separate edges $us$ and $sv$, and how to resolve $\vec{[h]}$ and $\alpha$ terms. To deal with the issue, we reduce distances to u and v as follows:
