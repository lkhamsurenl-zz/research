...
\usepackage{../packages/jeffe,../packages/enumitem,amsmath,listings, graphicx, 
caption, subcaption}
\oddsidemargin 0in
\evensidemargin 0in
\textwidth 6.5in
...
\begin{center}
{\color{red}
1. Explain our initial attempts on leafmost tree maintaining to show it is 
sufficient for dealing with non-unique shortest paths.\\
2. Show counterexample.}
\end{center}

\textbf{Difference of holiest Tree and leftmost tree:} \\
Our initial hope was that by computing only leftmost tree, we might be able to 
compute holy tree. However, on genus $g = 4$ surface, we show that no
configuration of homology and leafmost terms can result in equal holy and
